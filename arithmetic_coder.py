...
class ArithmeticCoderBase(object):
	...
	def state_dict(self):
		"""Return the minimal exact state needed to continue coding."""
		return {
			"num_state_bits": self.num_state_bits,
			"low": self.low,
			"high": self.high,
		}
	...
	def load_state_dict(self, state):
		if state["num_state_bits"] != self.num_state_bits:
			raise ValueError("Arithmetic coder state width does not match")
...
class ArithmeticEncoder(ArithmeticCoderBase):
	...
	def state_dict(self):
		state = super(ArithmeticEncoder, self).state_dict()
		state["num_underflow"] = self.num_underflow
		return state


	def load_state_dict(self, state):
		super(ArithmeticEncoder, self).load_state_dict(state)
		self.num_underflow = int(state["num_underflow"])
...
class ArithmeticDecoder(ArithmeticCoderBase):
	...
	def state_dict(self):
		state = super(ArithmeticDecoder, self).state_dict()
		state["code"] = self.code
		return state


	def load_state_dict(self, state):
		super(ArithmeticDecoder, self).load_state_dict(state)
		self.code = int(state["code"])
...
class BitInputStream(object):
	...
	def load_state_dict(self, state):
		self.input.seek(int(state["file_position"]))
		self.currentbyte = int(state["currentbyte"])
		self.numbitsremaining = int(state["numbitsremaining"])
...
class BitOutputStream(object):
	...
	def state_dict(self):
		return {
			"file_position": self.output.tell(),
			"currentbyte": self.currentbyte,
	...
	def load_state_dict(self, state):
		position = int(state["file_position"])
		self.output.seek(position)
		self.output.truncate(position)
		self.currentbyte = int(state["currentbyte"])
		self.numbitsfilled = int(state["numbitsfilled"])
